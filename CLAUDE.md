...
- **Tier color contract.** Player/manager tiers (`bronze | silver | gold | elite | legend` and the holo/cosmic effect tier) map to a fixed color palette. Tier color logic is currently duplicated between `RetroPlayerCard` and `ManagerCard` — when you touch either, lift it to `src/utils/tierColors.ts`.
